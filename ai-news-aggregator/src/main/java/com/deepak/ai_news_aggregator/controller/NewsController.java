...
public class NewsController {
    ...
    @GetMapping("/latest")
    public ResponseEntity<PaginatedNewsResponse> getLatestNews(
        ...
        if(articlePage.isEmpty()) {
            ...
            return ResponseEntity.ok(
                    PaginationUtil.fromPage(articlePage)
            );
        }
    }

    @GetMapping("/category/{category}")
    public ResponseEntity<PaginatedNewsResponse> getNewsByCategory(
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "pubDate"));
        Page<NewsArticle> articlePage = newsArticleRepository.findByCategoryIgnoreCase(category, pageable);
        if(articlePage.isEmpty()) {
    ...
    public ResponseEntity<PaginatedNewsResponse> getNewsBySource(
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "pubDate"));
        Page<NewsArticle> articlePage = newsArticleRepository.findBySourceIgnoreCase(source, pageable);
        if(articlePage.isEmpty()) {
            return ResponseEntity.badRequest().body(
                    PaginatedNewsResponse.builder()
                            .articles(Collections.emptyList())
    ...
    @PostMapping("/filter")
    public ResponseEntity<PaginatedNewsResponse> filterNews(
        ...
        try {
            ...
            long total = mongoTemplate.count(query, NewsArticle.class);

            Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "pubDate"));
            query.with(pageable);

            List<NewsArticle> pagedResults = mongoTemplate.find(query, NewsArticle.class);

            Page<NewsArticle> articlePage = new PageImpl<>(pagedResults, pageable, total);
